...
export function GatewayFormDialog({
  ...
  useEffect(() => {
    ...
    setKeySecret(gateway?.key_secret ?? "")
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      if (isEdit && gateway) {
        const payload: GatewayConfigUpdate = {
        ...
        if (keySecret.trim() && keySecret !== gateway.key_secret) {
          payload.key_secret = keySecret
        }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Razorpay API credentials. The secret is shown masked — click it and
            type to replace it, or leave it as is to keep the current value.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="grid gap-4">
          ...
          <div className="grid gap-2">
            <Label htmlFor="gw-secret">Key secret</Label>
            <MaskedSecretInput
              id="gw-secret"
              value={keySecret}
              onChange={setKeySecret}
              saved={gateway?.key_secret}
            />
          </div>
          ...
          <DialogFooter>
            ...
            <Button type="submit" disabled={saving || !keySecret.trim()}>
